...
# Run images/podman/tests/docker.sh from a NixOS guest that uses the real
# Docker daemon.
...
echo "==> copying repository into the guest"
ssh_guest mkdir -p /workspace
tar -C "$repo_root" -cf - . | ssh_guest tar -xf - -C /workspace

echo "==> running images/podman/tests/docker.sh in the guest"
ssh_guest env NIX_PATH="nixpkgs=/etc/nixpkgs" bash /workspace/images/podman/tests/docker.sh
